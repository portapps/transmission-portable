...
func main() {
	...
	app.Process = filepath.Join(app.AppPath, "transmission-qt.exe")

	// Cleanup on exit
	if cfg.Cleanup {
		defer func() {
			files.Cleanup(filepath.Join(os.Getenv("APPDATA"), "transmission"))
		}()
	}

	os.Setenv("TRANSMISSION_HOME", app.DataPath)
